...
public class ConnectToMongoDB {
    ...
    public static String insertIntoToMongoDB(User user){
        ...
        Document document = new Document().append("stName",user.getStName()).append("stID", user.getStID()).append("stDOB",user.getStDOB());
    ...
    public String insertIntoMongoDB(List<Student> student, String profileName){
        ...
        boolean collectionExists = mongoDatabase.listCollectionNames().into(new ArrayList<String>()).contains(profileName);
        if(collectionExists) {
            myCollection.drop();
        }
